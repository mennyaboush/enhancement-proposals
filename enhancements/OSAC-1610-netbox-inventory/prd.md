...
- **NetBox device management** — adding/removing devices, defining custom fields, and maintaining pool/capability data in NetBox remain the administrator's responsibility.
...
- **Pool-wide capacity/usage monitoring** — handled by a separate cross-backend observability feature.
...
- As a Cloud Infrastructure Admin, I want bare metal hosts to be selected according to the capability names and values I maintain in NetBox so that tenant requests are matched to appropriate hosts without encoding values into tag names.
...
- Enrolled hosts have valid, unique names that can be used unchanged by Metal3
  and any hostname-based fabric inventory. Administrators drain affected
  pending/allocated instances before renaming or replacing devices.
